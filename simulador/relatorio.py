...
def gerar_resumo_financeiro(
    ...
    return ResumoFinanceiro(
        ...
        data_ultima_parcela_paga=ultima["Data"],
        valor_ultima_parcela_paga=ultima["Valor Pago"],
        data_proxima_parcela=primeira_projetada["Data"],
...
def renderizar_relatorio_markdown(resumo: ResumoFinanceiro) -> str:
    ...
    return "\n".join(
        (
            ...
            f"- Última parcela paga: {resumo.data_ultima_parcela_paga:%d/%m/%Y} — {_moeda(resumo.valor_ultima_parcela_paga)}",
            f"- Próxima parcela: {resumo.data_proxima_parcela:%d/%m/%Y} — {_moeda(resumo.proxima_prestacao)}",
